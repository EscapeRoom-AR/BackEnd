...
use \Exception;
use \PDO;
use Model\Item as ChildItem;
use Model\ItemQuery as ChildItemQuery;
use Model\Map\ItemTableMap;
 ...
 *
 * @method     \Model\RoomQuery|\Model\HintQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
abstract class ItemQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \Model\Base\ItemQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\Model\\Item', $modelAlias = null)
    {
    ...
    /**
     * Filter the query by a related \Model\Room object
     *
     * @param \Model\Room|ObjectCollection $room The related object(s) to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @throws \Propel\Runtime\Exception\PropelException
     *
    ...
    public function filterByRoom($room, $comparison = null)
    {
        if ($room instanceof \Model\Room) {
            ...
            throw new PropelException('filterByRoom() only accepts arguments of type \Model\Room or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \Model\RoomQuery A secondary query class using the current class as primary query
     */
    public function useRoomQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinRoom($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Room', '\Model\RoomQuery');
    }

    /**
     * Filter the query by a related \Model\Hint object
     *
     * @param \Model\Hint|ObjectCollection $hint the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildItemQuery The current query, for fluid interface
     */
    public function filterByHint($hint, $comparison = null)
    {
        if ($hint instanceof \Model\Hint) {
            ...
            throw new PropelException('filterByHint() only accepts arguments of type \Model\Hint or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \Model\HintQuery A secondary query class using the current class as primary query
     */
    public function useHintQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinHint($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Hint', '\Model\HintQuery');
    }
